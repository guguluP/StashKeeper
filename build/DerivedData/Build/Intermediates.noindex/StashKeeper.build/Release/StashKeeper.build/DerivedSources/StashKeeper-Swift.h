...
#ifndef STASHKEEPER_SWIFT_H
#define STASHKEEPER_SWIFT_H
...
# undef any
# pragma clang attribute push(__attribute__((external_source_symbol(language="Swift", defined_in="StashKeeper",generated_declaration))), apply_to=any(function,enum,objc_interface,objc_category,objc_protocol))
...
SWIFT_CLASS("_TtC11StashKeeper16ConfettiRenderer")
@interface ConfettiRenderer : NSObject <MTKViewDelegate>
...
/// <code>renderer.isFinished</code> on a lightweight timer so the celebration overlay
/// can dismiss itself once every particle has faded, rather than staying
/// on screen (and the Metal view rendering) forever.
SWIFT_CLASS("_TtC11StashKeeper11Coordinator")
@interface Coordinator : NSObject <MTKViewDelegate>
- (void)mtkView:(MTKView * _Nonnull)view drawableSizeWillChange:(CGSize)size;
- (void)drawInMTKView:(MTKView * _Nonnull)view;
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
+ (nonnull instancetype)new SWIFT_UNAVAILABLE_MSG("-init is unavailable");
...
SWIFT_CLASS("_TtC11StashKeeper19NotificationManager")
...
@interface NotificationManager (SWIFT_EXTENSION(StashKeeper)) <UNUserNotificationCenterDelegate>
